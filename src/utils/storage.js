...
- 动词短语扩展： 将简洁的动词或动词短语替换为更长的描述（如“管理”->“开展...的管理工作”）。
- 增加辅助词/结构： 在句子中添加辅助词（如“了”、“的”、“地”、“所”、“会”、“可以”、“这个”、“方面”、“当中”等）。
2. 系统性词汇替换：
- 采用/使用 -> 运用/选用；基于 -> 鉴于/基于...来开展；利用 -> 借助/凭借；通过 -> 依靠；和/及/与 -> 以及。
- 原因 -> 缘由；符合 -> 契合；适合 -> 适宜；特点 -> 特性；极大(地) -> 极大程度(上)。
3. 括号内容处理：
- 解释性括号： 优先整合（使用 “也就是”、“即”、“比如”、“像” 等引导词）。
- 代码/标识符旁括号： 通常直接移除括号。
4. 句式微调与口语化倾向：
- 使用“把”字句；条件句式从“若...则...”改为“如果...就...”。
- 增加语气词/连接词： 如“那么”、“这样”、“同时”等。
绝对禁止修改技术术语及核心逻辑。直接输出修改后的文本。`,
    color: "#ec4899",
  },
  {
    id: "rewrite",
